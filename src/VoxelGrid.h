...
public:
	...
	Grid3uc toSubVolumeOccupancy(const vec3ui& center, int radius, unsigned int weightThresh, float sdfThresh, unsigned int& numKnown, float& minDist) const {
		unsigned int size = 2 * radius + 1;
		numKnown = 0;
		minDist = std::numeric_limits<float>::infinity();

		Grid3uc res(size, size, size);
		for (int k = -radius; k <= radius; k++) {
			for (int j = -radius; j <= radius; j++) {
				for (int i = -radius; i <= radius; i++) {
					int x = i + (int)center.x;	int y = j + (int)center.y;	int z = k + (int)center.z;
					if (x < 0 || y < 0 || z < 0 || x >= (int)m_dimX || y >= (int)m_dimY || z >= (int)m_dimZ) {
						res(i + radius, j + radius, k + radius) = 0; // empty (out of bounds)
					}
					else {
						float weight = (*this)(x, y, z).weight;
						float sdf = (*this)(x, y, z).sdf;
						if (sdf >= 0.0f && weight >= weightThresh) {
							float dist = sdf + vec3f((float)i, (float)j, (float)k).length();
							if (dist < minDist) {
								minDist = dist;
							}
						}
						if (weight >= weightThresh && sdf >= sdfThresh) {
							res(i + radius, j + radius, k + radius) = 0; // empty 
							numKnown++;
						}
						else if (weight >= weightThresh && sdf >= 0.0f && sdf < sdfThresh) {
							res(i + radius, j + radius, k + radius) = 1; // occupied
							numKnown++;
						}
						else if (weight < weightThresh || sdf < 0.0f) {
							res(i + radius, j + radius, k + radius) = 2; // unknown
						}
					}
				}
			}
	...
	void saveToFile(const std::string& filename, const vec3f& origDim) const {
		...
		outFile.write((const char*)&dimX, sizeof(UINT64));
		outFile.write((const char*)&dimY, sizeof(UINT64));
	...
	void loadFromFile(const std::string& filename) {
		if (util::endsWith(filename, ".sdf")) {
			...
			s.read((char*)&dimY, sizeof(UINT64));
			s.read((char*)&dimZ, sizeof(UINT64));
			s.read((char*)m_worldToGrid.getData(), sizeof(mat4f));
			allocate(dimX, dimY, dimZ);
			std::vector<float> sdfvalues(getNumElements());
			s.read((char*)sdfvalues.data(), sizeof(float)*sdfvalues.size());
			s.close();
